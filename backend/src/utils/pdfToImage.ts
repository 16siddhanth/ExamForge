 ...
 */
export async function renderPDFPageToImage(pdfPath: string, pageNumber: number): Promise<Buffer> {
        ...
        try {
            if (pdf) {
                try {
                    ...
                    if (typeof pdf.close === 'function') pdf.close();
                } catch (e) {
 ...
 * @returns Promise with PNG buffer
 */
export async function createPDFPagePreview(pdfPath: string, pageNumber: number): Promise<Buffer> {
    let data: Buffer | null = null;
    let pdf: any = null;
