...
router.get('/:userId', async (req, res) => {
    try {
        ...
        const quizWithParsedQuestions = quizzes.map((quiz) => {
            try {
                return {
                    ...quiz,
                ...
                return {
                    ...quiz,
                    questions: { questions: [], timeSpent: 0, subjectId: quiz.id }
                };
            }
        });
...
router.post('/generate', async (req, res) => {
    try {
        ...
        const existingQuestions = subject.documents
            ...
            .filter((q) => {
            if (!topic)
                return true; // Include all questions if no topic specified
        ...
        const questionsByTopic = existingQuestions.reduce((acc, q) => {
        ...
        try {
            ...
            ${Object.entries(questionsByTopic)
                                    .map(([topicName, questions]) => (`Topic: ${topicName}\nExample questions:\n${questions
                                    .slice(0, 2)
                                    .map((q) => `- ${q.text}`)
                                    .join('\n')}`))
                                    .join('\n\n')}
